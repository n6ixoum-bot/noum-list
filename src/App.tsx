...
function App() {
      ...
      <>
        ...
        <section className="three-grid">
          <Panel title={copy.paths} subtitle={copy.current} action={<button className="text-button" onClick={() => changeView("paths")}>{copy.viewAll} <ChevronLeft size={16} /></button>}>
            {state.paths.length ? <div className="compact-path-list">{state.paths.slice(0, 3).map((path) => <PathMini key={path.id} path={path} locale={locale} onContinue={() => changeView("paths")} />)}</div> : <EmptyState icon={<FolderKanban size={22} />} text={isArabic ? "أنشئ مسارك الأول." : "Create your first path."} />}
          </Panel>
  ...
  function renderPaths() {
    return <>
      ...
      {state.paths.length ? <div className="path-grid">{state.paths.map((path) => <PathCard key={path.id} path={path} locale={locale} onContinue={() => { changeView("dashboard"); notify(path.nextAction); }} />)}</div> : <EmptyState icon={<FolderKanban size={22} />} text={isArabic ? "لا توجد مسارات بعد. أنشئ مسارك الأول للبدء." : "No paths yet. Create your first path to begin."} />}
  ...
  function renderBrain() {
    return <>
      ...
      <div className="brain-layout">
        <Panel title={copy.notes} className="notes-index">{state.notes.length ? <div className="note-index-list">{state.notes.map((note) => <button type="button" className={`note-index-item ${note.id === activeNote?.id ? "active" : ""}`} key={note.id} onClick={() => setSelectedNoteId(note.id)}><span className="note-icon"><NotebookPen size={16} /></span><span><strong>{note.title}</strong><small>{note.updatedAt}</small></span><ChevronLeft size={16} /></button>)}</div> : <EmptyState icon={<NotebookPen size={22} />} text={isArabic ? "لا توجد ملاحظات بعد." : "No notes yet."} />}</Panel>
  ...
  function renderLearning() {
    ...
    return <>
      ...
      <Panel title={isArabic ? "حزمة المفردات" : "Vocabulary pack"} subtitle={isArabic ? "أضف بطاقاتك الأولى للبدء." : "Add your first cards to begin."}>{state.flashcards.length ? <div className="vocab-grid">{state.flashcards.map((card) => <article key={card.id} className={`vocab-card ${card.due ? "due" : ""}`}><span>{card.language}</span><h3>{card.front}</h3><p>{card.back}</p><small>{card.due ? `${copy.review} · ${card.interval}d` : (isArabic ? "مجدولة لاحقًا" : "Scheduled later")}</small></article>)}</div> : <EmptyState icon={<BrainCircuit size={22} />} text={isArabic ? "لا توجد بطاقات بعد." : "No flashcards yet."} />}</Panel>
    </>;
  }
  ...
  function renderFocus() {
    return <>
      ...
      <section className="focus-history"><Panel title={isArabic ? "آخر الجلسات" : "Recent sessions"}><EmptyState icon={<TimerReset size={22} />} text={isArabic ? "ستظهر جلساتك هنا بعد أول جلسة تركيز." : "Your sessions will appear here after your first focus session."} /></Panel></section>
    </>;
  }
